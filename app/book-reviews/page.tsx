...
import { getBookReviews } from "@/lib/request";
...
export default async function BookReviewsPage({
  ...
  const allReviews = await getBookReviews();
      ...
      <section className="section-spacing border-b border-border/70">
        <div className="container-fixed">
          <div className="mb-10 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <p className="text-xs font-black capitalize tracking-[0.3em] mb-3">Book Reviews</p>
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-black capitalize" style={{ letterSpacing: "-0.03em" }}>
                Detailed reviews & reading notes
              </h1>
            </div>
            <Link
              href="/"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12">
            {paginatedReviews.map((review) => {
              ...
              return (
                <article key={review._id} className="group flex h-full flex-col border border-border/70 bg-card p-4 text-card-foreground transition-colors hover:border-foreground/40">
                  <div className="relative aspect-[4/3] mb-6 overflow-hidden bg-muted">
                    <img src={imageUrl} alt={review.bookTitle} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />
                  </div>

                  <div className="mb-4">
                    <span className="pill-badge text-[10px]">{review.genre?.toUpperCase() || "BOOK REVIEW"}</span>
                  </div>
                  ...
                  <p className="mb-4 text-sm leading-[1.6] line-clamp-3">
                    {review.shortSummary || review.excerpt || "A detailed review is available on this page."}
                  </p>
                  ...
                  <Link href={`/book-reviews/${review.slug}`} className="mt-6 inline-flex items-center gap-2 text-sm font-black capitalize tracking-[0.2em] hover:opacity-70 transition-opacity">
            ...
            <nav className="mt-16 flex flex-wrap items-center justify-center gap-3" aria-label="Pagination">
              {Array.from({ length: totalPages }, (_, index) => {
                ...
                return (
                  <Link
                    key={pageNumber}
                    href={{ pathname: "/book-reviews", query: pageNumber === 1 ? {} : { page: pageNumber } }}
